...
- [Demos.juc.CountDownLatchDemo/CyclicBarrier/Semaphore](#countdownlatchcyclicbarriersemaphore)
  - [Demos.juc.CountDownLatchDemo](#countdownlatch)
    - [枚举类的使用](#枚举类的使用)
...
```java
public class Demos.juc.SingletonDemo {
    private static Demos.juc.SingletonDemo singletonDemo=null;
    private Demos.juc.SingletonDemo(){
        System.out.println(Thread.currentThread().getName()+"\t 我是构造方法");
    }
    //DCL模式 Double Check Lock 双端检索机制：在加锁前后都进行判断
    public static Demos.juc.SingletonDemo getInstance(){
        if (singletonDemo==null){
            synchronized (Demos.juc.SingletonDemo.class){
                 if (singletonDemo==null){
                     singletonDemo=new Demos.juc.SingletonDemo();
                 }
            }
        }
    ...
    public static void main(String[] args) {
        for (int i = 0; i < 10; i++) {
            new Thread(()->{
                Demos.juc.SingletonDemo.getInstance();
...
```java
public class Demos.juc.CASDemo {
    public static void main(String[] args) {
...
[Demos.juc.ReadWriteLockDemo](https://github.com/MaJesTySA/JVM-JUC-Core/blob/master/src/thread/ReadWriteLockDemo.java)
...
`Demos.juc.CountDownLatchDemo`的问题是**不能复用**。比如`count=3`，那么加到3，就不能继续操作了。而`Semaphore`可以解决这个问题，比如6辆车3个停车位，对于`Demos.juc.CountDownLatchDemo`**只能停3辆车**，而`Semaphore`可以停6辆车，车位空出来后，其它车可以占有，这就涉及到了`Semaphore.accquire()`和`Semaphore.release()`方法。
...
```java
public class Demos.juc.CallableDemo {
    //实现Callable接口
    class Demos.juc.MyThread implements Callable<Integer> {
        @Override
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        FutureTask<Integer> futureTask = new FutureTask<>(new Demos.juc.MyThread());
...
```java
9688 thread.Demos.juc.DeadLockDemo
